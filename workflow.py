...
"""workflow.py -- run the full certification workflow.
    ...
    python workflow.py INSTANCE.json         certify only (seconds).
    python workflow.py INSTANCE.json --deep  certify, then a fresh mutation
                                             run, then (coverage, one
                                             variable) the mutant census.
                                             Minutes per instance.
    python workflow.py --library             certify every instance in the
                                             library/ folder, controls
                                             included. Fast; run after any
                                             edit.
    python workflow.py --library DIR         same, for instances in DIR.
    python workflow.py --all                 legacy: instances in the
                                             current directory.
    Add --deep to --library or --all for the full pipeline on every
    instance. Slow; a pre-release job, not a routine one.
